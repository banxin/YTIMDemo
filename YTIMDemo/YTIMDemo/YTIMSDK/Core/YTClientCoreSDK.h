 ...
 * @version 2.1
 */
@interface YTClientCoreSDK : NSObject
...
/*!
 * 保存提交到服务端的唯一id，保证唯一就可以通信，可能是登陆用户名
...
@property (nonatomic, retain) id<YTChatTransDataEventProtocol> chatTransDataEvent;
/*! 通用数据通信消息的回调事件（如：收到聊天数据事件通知、服务端返回的错误信息事件通知） */
@property (nonatomic, retain) id<YTChatBaseEventProtocol> chatBaseEvent;
